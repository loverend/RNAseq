...
do
  ...
  echo "#$ -q short.qc" >> $exc_name
  echo "#$ -hold_jid filter.$sample" >> $exc_name
  ...
  echo "export PYTHONPATH=/well/jknight/Irina/Programs/HTSeq-0.6.1/build/lib.linux-x86_64-2.7:/well/jknight/software/rescomp/lib/python2.7/site-packages/${PYTHONPATH:+:$PYTHONPATH}" >> $exc_name
        ...
        #tmp="rm $BASEDIR/Filtering/Filteringscripts/filtering.$sample.sh"
        #echo $tmp >> $exc_name  

cmd="/well/immune-rep/shared/CODE/RNASeqSTARPipeline/htseq-count --type=exon --order=name --mode=union --stranded=reverse --format=bam --idattr=gene_name $BASEDIR/Filtering/"$sample"/"$sample".$FORCOUNTS.namesorted.bam $GTF > $OUTPUT_DIR/$sample.counts.txt"
  echo $cmd >> $exc_name 

        tmp="rm $BASEDIR/Filtering/$sample/*bam"
        echo $tmp >> $exc_name

	tmp="rm $BASEDIR/Mapping/$sample/*bam"
	echo $tmp >> $exc_name
